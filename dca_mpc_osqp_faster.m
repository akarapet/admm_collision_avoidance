...
for p = 1:nsim
    ...
    for i =1:M
        ...
        ctrl(:,i) = res(:,i).x((N+1)*nx+1:(N+1)*nx+nu);
        x0(i,:) = A*x0(i,:)' + B*ctrl_prev(:,i);
        ctrl_prev(:,i) = ctrl(:,i);
        implementedX{i} = [implementedX{i}; x0(i,:)'];
        % Update initial state
        leq(1:nx,i) = -x0(i,:);
        ueq(1:nx,i) = -x0(i,:);
        prob(i).update('l', [leq(:,i);min_input],'u', [ueq(:,i);max_input])
    end
...
for k = 1:nsim+1
    for i = 1:M
        ...
        xconcat = [xconcat;implementedX{i}((k-1)*nx+1:k*nx)];
